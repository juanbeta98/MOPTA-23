...
for test in ["A","B","C"]:
    ...
    fixed_cost,overtime_cost, travel_time_cost = 0.0,0.0,0.0
    ...
    for i in range(len(final_routes_indices)):
        ...
        travel_time_cost+=time*c_t
        if time>L:
            overtime_cost+=(time-L)*c_o
        print(path, cost, time) 
    
    fixed_cost = len(final_routes)*c_f
    number_of_routes = len(final_routes)
    ...
    for (route,cost,time) in final_routes:
        ...
        arrival_times_depot[test].append(w[route[-2]] + mean_service_time + t[route[-2],n+1])
    ...
    sys.stdout = open("./Experiment2(alpha).txt", "a")
    print(test, n, alpha, round(total_time,2), sep =",")
    sys.stdout = original

for test in ["A-B","A-C","B-C"]:    
    ############### PRINT RESULTS ##################
    if test == "A-B":
        test1 = "A"
        test2 = "B"
        ...
        test2 = "C"
    original = sys.stdout
    sys.stdout = open("./Experiment2(alpha-details).txt", "a")
    result = [arrival_times_depot[test1][i]-arrival_times_depot[test2][i]  for i in range(len(arrival_times_depot[test1]))]
    print(test, n, round(sum(result)/len(result),2), sep =",")
    sys.stdout = original
