...
draw_nodes(map_model,nodes_dict)
COLOR_ACT = COLORS.copy()     
for path,cost,time in final_routes:
    col = random.choice(COLOR_ACT)
    COLOR_ACT.remove(col)
    map_model = draw_arc_route_aux(map_model, path, col)
...
sys.stdout = open("./OutputData.txt", "a")
print("Arcs(l,u):=data")
...
############### PRINT RESULTS (Input)##################
sys.stdout = open("./InputData.txt", "a")
print("Locations:=data{0", *N,sep= ",", end = "}; \n")
print("XCoordinate(l):=data{", end = "\n")
for j in V:
    if j!=n+1:
        print(j,":",nodes_dict[j][0], sep = "", end = ",")
print("\n","};", sep = "", end = "\n")
print("YCoordinate(l):=data{", end = "\n")

for j in V:
    if j!=n+1:
        print(j,":",nodes_dict[j][1], sep = "", end = ",")
