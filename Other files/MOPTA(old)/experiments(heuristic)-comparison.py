...
#Arcos
A = list(d.keys())     
for t_max in range(2):
    #t_max = 0 -> expectation
    #t_max = 1 -> robustness
    
    service_time = 45
    if t_max == 1:
        service_time = stats.expon.ppf(0.95, scale = service_time)
    ...
    for i,j in A:
        ...
        if t_max == 0:
            t[(i,j)] = round(math.exp(mu + sigma**2/2), decimals) #expected value
        else:
            t[(i,j)]  = round(stats.lognorm.ppf(0.95, s = sigma, scale = math.exp(mu)), decimals)
        mu_aux[(i,j)] = mu
        sigma_aux[(i,j)] = sigma
    ...
    for (route,cost,time) in final_routes:
        ...
        for k in range(1,REPLICATIONS+1):
            ...
            for arc in route_arcs:
                if np.random.binomial(1, 0.05) == 0: 
                    arrival_time += np.random.lognormal(mu_aux[arc],sigma_aux[arc],size=1)[0]
                    replication_info_by_node[arc[1]].append(arrival_time)
                    arrival_time+=np.random.exponential(scale = 45, size = 1)[0]
        for i in route:
            if i>=1 and i<=n:
                w[i] = np.percentile(replication_info_by_node[i], alpha*100)
    ...
    for k in range(1,REPLICATIONS+1):
        ...
        fixed_cost = len(final_routes)*c_f
        overtime_cost = 0.0
        on_time_services = 0
        late_services = 0
        waiting_time = 0
        ...
        for (route,cost,time) in final_routes:
            ...
            for arc in route_arcs:
                if np.random.binomial(1, 0.05) == 0: 
                    travel_time = np.random.lognormal(mu_aux[arc],sigma_aux[arc],size=1)[0]
                    travel_time_cost+= travel_time*c_t
                    arrival_time += travel_time
                    if arc[1]<=n:
                        print(arrival_time, " - ", w[arc[1]])
                        if arrival_time<=w[arc[1]]:
                            on_time_services+=1
                            idle_time+=(w[arc[1]]-arrival_time)
                        else:
                            late_services+=1
                            waiting_time+=(arrival_time-w[arc[1]])
                        arrival_time+= np.random.exponential(scale = 45, size = 1)[0]
                else:
                    cancellations+=1
            if arrival_time-L>0:
                overtime_cost+=(arrival_time-L)*c_o
        total_cost = fixed_cost + travel_time_cost + overtime_cost
        original = sys.stdout
        sys.stdout = open("./Experiment4.txt", "a")
        print(n,k, t_max, round(total_cost,2),on_time_services, late_services,len(final_routes), round(idle_time,2), round(waiting_time,2),(n-cancellations), sep =",")
        sys.stdout = original
