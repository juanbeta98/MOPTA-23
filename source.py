...
def test_scenario_sensitivity(S,K,K_s,n,a,t):
    ...
    st_conv = {}
    for s in S:
        st_conv[s] = mp.addConstr(aux[s] <= n[s], f"S{s}_convexity")

    mp.setParam("OutputFlag",0)

    time0 = process_time(); i = 0
    lbd = {s:list() for s in S}
    columns = {s:list() for s in S}
    ...
    while True:
        ...
        for s in S:
            ...
            for l in routes_DFS:
                col = {k:1 if k in l else 0 for k in K}
                new_Col = gb.Column(col.values(),vehic_assign.values())
                lbd[s].append(mp.addVar(vtype=gb.GRB.CONTINUOUS,name=f"lbd_{s}_{len(lbd[s])}",obj=0,ub=1,column=new_Col))
                columns[s].append(l)

                mp.chgCoeff(st_conv[s],lbd[s][-1],1)
